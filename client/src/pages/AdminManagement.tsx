...
const AdminManagement: React.FC = () => {
    ...
    const getBookingStatusLabel = (booking: any) => {
        ...
        return booking.approvalStatus || booking.status || 'pending';
    };

    const getBookingStatusClasses = (booking: any) => {
        if (booking.approvalStatus === 'rejected') return 'bg-red-50 text-red-700';
        if (booking.extensionStatus === 'requested') return 'bg-amber-50 text-amber-700';
        if (booking.provisioningStatus === 'provisioned') return 'bg-primary/10 text-primary/80';
        if (booking.approvalStatus === 'approved') return 'bg-sky-50 text-sky-700';
        return 'bg-slate-100 text-slate-500';
    };

    const openBookingControl = (booking: any) => {
        setSelectedBooking({
            ...booking,
            approvalStatus: booking.approvalStatus || 'pending',
            provisioningType: booking.provisioningType || 'aws',
            provisioningStatus: booking.provisioningStatus || 'not_started',
            extensionStatus: booking.extensionStatus || 'none',
            expiresAt: booking.expiresAt ? new Date(booking.expiresAt).toISOString().slice(0, 16) : '',
            deletionScheduledAt: booking.deletionScheduledAt ? new Date(booking.deletionScheduledAt).toISOString().slice(0, 16) : '',
            localProvisioning: {
                templateName: '',
                vmId: '',
                nodeName: '',
            ...
            awsProvisioning: {
                launchUrl: booking.awsProvisioning?.launchUrl || booking.provisionedUrl || '',
                accountLabel: booking.awsProvisioning?.accountLabel || ''
            }
        });
        setIsBookingModalOpen(true);
    };

    const saveBookingControl = async (overrides: any = {}) => {
        if (!selectedBooking) return;
        setBookingActionLoading(true);
        try {
            const payload = {
                approvalStatus: selectedBooking.approvalStatus,
                provisioningType: selectedBooking.provisioningType,
                provisioningStatus: selectedBooking.provisioningStatus,
                adminNote: selectedBooking.adminNote || '',
                expiresAt: selectedBooking.expiresAt ? new Date(selectedBooking.expiresAt).toISOString() : undefined,
                deletionScheduledAt: selectedBooking.deletionScheduledAt ? new Date(selectedBooking.deletionScheduledAt).toISOString() : undefined,
                localProvisioning: selectedBooking.provisioningType === 'local' ? selectedBooking.localProvisioning : undefined,
                awsProvisioning: selectedBooking.provisioningType === 'aws' ? selectedBooking.awsProvisioning : undefined,
            ...
            if (!res.ok) {
                alert(data.message || data.error || 'Booking update failed');
                return;
            }
            alert(data.message || 'Action successful');
            setIsBookingModalOpen(false);
            fetchData();
        } catch (e: any) { console.error(e); alert(`Connection error: ${e.message}`); } finally { setBookingActionLoading(false); }
    };
    ...
    const renderOverviewTab = () => (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm"><div className="flex items-center gap-4 mb-2"><div className="p-3 bg-primary/10 rounded-2xl text-primary"><Users className="w-6 h-6" /></div><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Users</span></div><p className="text-3xl font-black text-slate-900">{stats?.users || 0}</p></div>
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm"><div className="flex items-center gap-4 mb-2"><div className="p-3 bg-primary/5 rounded-2xl text-primary"><Activity className="w-6 h-6" /></div><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Active</span></div><p className="text-3xl font-black text-slate-900">{stats?.activeSessions || 0}</p></div>
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm"><div className="flex items-center gap-4 mb-2"><div className="p-3 bg-secondary/10 rounded-2xl text-secondary"><FileCheck className="w-6 h-6" /></div><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tasks</span></div><p className="text-3xl font-black text-slate-900">{stats?.submissions || 0}</p></div>
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm"><div className="flex items-center gap-4 mb-2"><div className="p-3 bg-accent/10 rounded-2xl text-accent"><MessageSquare className="w-6 h-6" /></div><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Feedback</span></div><p className="text-3xl font-black text-slate-900">{stats?.feedback || 0}</p></div>
            </div>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col"><div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between"><h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-3"><History className="w-5 h-5 text-primary" />Summary</h3></div><div className="p-6 space-y-4">{logs.slice(0, 5).map((l: any) => (<div key={l._id} className="flex gap-4 p-4 bg-slate-50/50 rounded-2xl border border-slate-100"><div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${l.severity === 'error' ? 'bg-red-500' : 'bg-primary'}`} /><div><p className="text-xs font-bold text-slate-700">{l.message}</p></div></div>))}</div></div>
                <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden"><div className="px-8 py-6 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3"><TrendingUp className="w-5 h-5 text-primary" /><h3 className="text-sm font-black text-slate-900 uppercase tracking-widest">System Pulse</h3></div><div className="p-8 space-y-6"><div className="flex justify-between items-center text-[10px] font-black uppercase"><span className="text-slate-400">Memory</span><span className="text-primary">{health?.memory?.percentage || '0%'}</span></div><div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden"><div className="bg-primary/80 h-full transition-all duration-1000" style={{ width: health?.memory?.percentage }} /></div></div></div>
            </div>
        </div>
    );
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
                <div className="overflow-x-auto"><table className="w-full"><thead className="bg-slate-50 border-b border-slate-200"><tr><th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Terminated identity</th><th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Reason</th><th className="px-8 py-6 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Actions</th></tr></thead>
                    <tbody className="divide-y divide-slate-100">{Array.isArray(data) && data.map((u: any) => (<tr key={u._id} className="hover:bg-red-50/30 transition-colors"><td className="px-8 py-6"><div className="font-black text-slate-900">{u.name}</div><div className="text-[10px] text-slate-400 font-bold uppercase">{u.email}</div></td><td className="px-8 py-6 text-xs font-bold text-slate-600 italic">"{u.deletionReason || 'N/A'}"</td><td className="px-8 py-6 text-right"><div className="flex gap-4 justify-end"><button onClick={() => handleRestore(u._id)} className="text-primary font-black text-[10px] uppercase transition-all flex items-center gap-2"><RefreshCw className="w-3 h-3" />Restore</button><button onClick={() => { setUserToDelete(u); setDeleteReason(''); setDeleteType('permanent'); setIsDeleteModalOpen(true); }} className="text-red-600 font-black text-[10px] uppercase transition-all flex items-center gap-2"><Trash2 className="w-3 h-3" />Purge</button></div></td></tr>))}</tbody>
                </table></div>
            </div>
        </div>
    );
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                <div className="flex items-center gap-4">
                    <input type="checkbox" checked={selectedIds.length === data.length && data.length > 0} onChange={toggleSelectAll} className="w-5 h-5 rounded-lg border-slate-300 text-primary focus:ring-primary/80" />
            ...
            {Array.isArray(data) && data.map((f: any) => (
                <div key={f._id} className={`bg-white rounded-[2.5rem] p-8 border transition-all relative group flex gap-6 ${selectedIds.includes(f._id) ? 'border-primary/60 bg-primary/5/10' : 'border-slate-200 shadow-sm hover:border-emerald-500/50'}`}>
                    <div className="pt-2"><input type="checkbox" checked={selectedIds.includes(f._id)} onChange={() => toggleSelect(f._id)} className="w-5 h-5 rounded-lg border-slate-300 text-primary focus:ring-primary/80" /></div>
                    <div className="flex-1">
                        <div className="flex justify-between items-start mb-6">
                            <div className="flex items-center gap-6">
                                <div className="w-14 h-14 bg-primary/5 rounded-3xl flex items-center justify-center"><MessageSquare className="w-7 h-7 text-primary" /></div>
    ...
    const renderSubmissionsTab = () => (
        <div className="bg-white rounded-[2.5rem] border border-slate-200 overflow-hidden shadow-sm">
            <div className="overflow-x-auto"><table className="w-full"><thead className="bg-slate-50 border-b border-slate-200"><tr><th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Researcher</th><th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Project</th><th className="px-8 py-6 text-left text-[10px] font-black text-slate-400 uppercase tracking-widest">Status</th><th className="px-8 py-6 text-right text-[10px] font-black text-slate-400 uppercase tracking-widest">Review</th></tr></thead>
                <tbody className="divide-y divide-slate-100">{Array.isArray(data) && data.map((s: any) => (<tr key={s._id} className="hover:bg-slate-50/50 transition-colors"><td className="px-8 py-6"><div className="font-black text-slate-900 tracking-tight">{s.student?.name}</div><div className="text-[10px] text-slate-400 font-bold uppercase">{s.student?.email}</div></td><td className="px-8 py-6"><div className="text-xs font-black text-slate-700 uppercase tracking-tight">{s.title}</div><div className="text-[10px] text-primary font-black uppercase tracking-tighter">{s.lab?.name}</div></td><td className="px-8 py-6"><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase ${s.gradingStatus === 'graded' ? 'bg-primary/10 text-primary/80' : 'bg-primary/5 text-primary/90'}`}>{s.gradingStatus}</span></td><td className="px-8 py-6 text-right"><div className="flex gap-4 justify-end"><button onClick={() => { const f = s.files[0]; if (f) handleDownload(s._id, 0, f.name); }} className="text-primary hover:text-primary/80 font-black text-[10px] uppercase transition-all flex items-center gap-2"><Download className="w-4 h-4" /> Download</button></div></td></tr>))}</tbody>
            </table></div>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-slate-50/30 text-slate-900 flex flex-col">
            <header className="bg-white border-b border-slate-200 sticky top-0 z-40 backdrop-blur-md bg-white/80"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5"><div className="flex items-center justify-between"><div className="flex items-center gap-6"><button onClick={() => navigate('/dashboard')} className="p-3 hover:bg-slate-50 border border-slate-100 rounded-2xl transition-all active:scale-95 shadow-sm"><ArrowLeft className="w-5 h-5 text-slate-600" /></button><div><h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">ACETEL Virtual Laboratory</h1><p className="text-primary text-[10px] font-black uppercase tracking-[0.2em] mt-0.5">Administrative Authorization Active</p></div></div><div className="hidden lg:flex items-center gap-10"><div className="flex items-center gap-3 px-6 py-2.5 bg-slate-900 text-white rounded-2xl shadow-xl shadow-slate-900/10"><Shield className="w-4 h-4 text-primary" /><span className="text-[10px] font-black uppercase tracking-widest">Level 4 Access</span></div></div></div></div></header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex flex-col lg:flex-row gap-10">
                <aside className="w-full lg:w-72 space-y-3">
                    ...
                    <button onClick={() => setActiveTab('audit_logs')} className={`w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group ${activeTab === 'audit_logs' ? 'bg-primary text-white shadow-2xl translate-x-2' : 'text-slate-500 hover:bg-white hover:text-primary'}`}><div className="flex items-center gap-4"><History className="w-5 h-5" />Audit Trails</div></button>
                    ...
                    {isFacilitator && <button onClick={() => setIsFacilitatorBookingOpen(true)} className="w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group text-slate-500 hover:bg-white hover:text-primary"><div className="flex items-center gap-4"><Calendar className="w-5 h-5" />Book Lab</div></button>}
                    <div className="py-4 px-8"><p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Parameters</p></div>
                    {hasPerm('manage_roles') && <button onClick={() => setActiveTab('roles')} className={`w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group ${activeTab === 'roles' ? 'bg-primary text-white shadow-2xl translate-x-2' : 'text-slate-500 hover:bg-white hover:text-primary'}`}><div className="flex items-center gap-4"><Shield className="w-5 h-5" />Role Matrix</div></button>}
                    {hasPerm('view_submissions') && <button onClick={() => window.open(NOUN_ELEARN_URL, '_blank')} className="w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group text-slate-500 hover:bg-white hover:text-primary"><div className="flex items-center gap-4"><FileCheck className="w-5 h-5" />Submissions</div></button>}
                    {hasPerm('view_feedback') && <button onClick={() => setActiveTab('feedback')} className={`w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group ${activeTab === 'feedback' ? 'bg-primary text-white shadow-2xl translate-x-2' : 'text-slate-500 hover:bg-white hover:text-primary'}`}><div className="flex items-center gap-4"><MessageSquare className="w-5 h-5" />Feedback</div></button>}
                    {hasPerm('manage_settings') && <button onClick={() => setActiveTab('settings')} className={`w-full flex items-center justify-between px-8 py-5 rounded-[1.5rem] font-black text-[11px] uppercase tracking-widest transition-all group ${activeTab === 'settings' ? 'bg-primary text-white shadow-2xl translate-x-2' : 'text-slate-500 hover:bg-white hover:text-primary'}`}><div className="flex items-center gap-4"><Settings className="w-5 h-5" />Settings</div></button>}
                </aside>
                <section className="flex-1 min-w-0">{isLoading ? <div className="h-[600px] bg-white rounded-[3rem] border border-slate-200 shadow-sm flex flex-col items-center justify-center gap-6"><Loader2 className="w-12 h-12 text-primary animate-spin" /><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Authorized Access Ongoing...</p></div> : <div className="animate-in fade-in slide-in-from-right-4 duration-700">
            ...
            {isBookingModalOpen && selectedBooking && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-slate-950/60 backdrop-blur-md" onClick={() => !bookingActionLoading && setIsBookingModalOpen(false)} /><div className="relative w-full max-w-3xl bg-white border border-slate-200 rounded-[3rem] shadow-2xl p-10 flex flex-col max-h-[90vh] overflow-y-auto"><div className="flex justify-between items-center mb-10"><div><h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Reservation Control</h2><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">{selectedBooking.user?.name} • {selectedBooking.lab?.name}</p></div><button onClick={() => setIsBookingModalOpen(false)} className="p-4 hover:bg-slate-50 rounded-2xl transition-all shadow-sm active:scale-95"><X className="w-6 h-6 text-slate-400" /></button></div>
                <div className="space-y-8 mb-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Booking Window</p>
                            <p className="text-sm font-black text-slate-900">{new Date(selectedBooking.startTime).toLocaleString()}</p>
                            <p className="text-sm font-bold text-slate-500 mt-1">to {new Date(selectedBooking.endTime).toLocaleString()}</p>
                        </div>
                        <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Current Workflow State</p>
                            <p className="text-sm font-black text-slate-900 uppercase">{getBookingStatusLabel(selectedBooking)}</p>
                            <p className="text-[10px] font-bold text-slate-500 mt-2">Approval: {selectedBooking.approvalStatus || 'pending'} • Provisioning: {selectedBooking.provisioningStatus || 'not_started'}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Approval Status</label>
                            <select value={selectedBooking.approvalStatus || 'pending'} onChange={(e) => setSelectedBooking({ ...selectedBooking, approvalStatus: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Provisioning Type</label>
                            <select value={selectedBooking.provisioningType || 'aws'} onChange={(e) => setSelectedBooking({ ...selectedBooking, provisioningType: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold">
                                <option value="aws">AWS</option>
                                <option value="local">Local VM</option>
                            </select>
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Provisioning Status</label>
                            <select value={selectedBooking.provisioningStatus || 'not_started'} onChange={(e) => setSelectedBooking({ ...selectedBooking, provisioningStatus: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Admin / Technician Note</label>
                            <input value={selectedBooking.adminNote || ''} onChange={(e) => setSelectedBooking({ ...selectedBooking, adminNote: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Expires At</label>
                            <input type="datetime-local" value={selectedBooking.expiresAt || ''} onChange={(e) => setSelectedBooking({ ...selectedBooking, expiresAt: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" />
                        </div>
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">Delete After Grace Period</label>
                            <input type="datetime-local" value={selectedBooking.deletionScheduledAt || ''} onChange={(e) => setSelectedBooking({ ...selectedBooking, deletionScheduledAt: e.target.value })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" />
                        </div>
                    </div>

                    {selectedBooking.provisioningType === 'aws' ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">AWS Launch URL</label>
                                <input value={selectedBooking.awsProvisioning?.launchUrl || ''} onChange={(e) => setSelectedBooking({ ...selectedBooking, awsProvisioning: { ...(selectedBooking.awsProvisioning || {}), launchUrl: e.target.value } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" />
                            </div>
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase ml-1">AWS Account Label</label>
                                <input value={selectedBooking.awsProvisioning?.accountLabel || ''} onChange={(e) => setSelectedBooking({ ...selectedBooking, awsProvisioning: { ...(selectedBooking.awsProvisioning || {}), accountLabel: e.target.value } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" />
                            </div>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">SSH Port</label><input type="number" value={selectedBooking.localProvisioning?.sshPort || 22} onChange={(e) => setSelectedBooking({ ...selectedBooking, localProvisioning: { ...(selectedBooking.localProvisioning || {}), sshPort: parseInt(e.target.value || '22', 10) } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" /></div>
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">CPU Cores</label><input type="number" value={selectedBooking.localProvisioning?.cpuCores || 2} onChange={(e) => setSelectedBooking({ ...selectedBooking, localProvisioning: { ...(selectedBooking.localProvisioning || {}), cpuCores: parseInt(e.target.value || '2', 10) } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" /></div>
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Memory (MB)</label><input type="number" value={selectedBooking.localProvisioning?.memoryMb || 2048} onChange={(e) => setSelectedBooking({ ...selectedBooking, localProvisioning: { ...(selectedBooking.localProvisioning || {}), memoryMb: parseInt(e.target.value || '2048', 10) } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" /></div>
                            <div className="space-y-2"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">Disk (GB)</label><input type="number" value={selectedBooking.localProvisioning?.diskGb || 20} onChange={(e) => setSelectedBooking({ ...selectedBooking, localProvisioning: { ...(selectedBooking.localProvisioning || {}), diskGb: parseInt(e.target.value || '20', 10) } })} className="w-full bg-white border border-slate-200 rounded-2xl px-6 py-4 text-slate-900 font-bold" /></div>
                        </div>
                    )}

                    {selectedBooking.extensionStatus === 'requested' && (
                        <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 space-y-3">
                            <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest">Pending Extension Request</p>
                            <p className="text-sm font-bold text-slate-700">Requested Until: {selectedBooking.extensionRequestedUntil ? new Date(selectedBooking.extensionRequestedUntil).toLocaleString() : 'N/A'}</p>
                            <p className="text-sm text-slate-600">Reason: {selectedBooking.extensionReason || 'No reason provided'}</p>
                            <div className="flex gap-4">
                                <button onClick={() => saveBookingControl({ extensionDecision: 'approved', extensionReviewNote: selectedBooking.adminNote || 'Extension approved' })} disabled={bookingActionLoading} className="flex-1 py-4 bg-primary text-white rounded-2xl font-black text-[10px] uppercase">Approve Extension</button>
                                <button onClick={() => saveBookingControl({ extensionDecision: 'rejected', extensionReviewNote: selectedBooking.adminNote || 'Extension rejected' })} disabled={bookingActionLoading} className="flex-1 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase">Reject Extension</button>
                            </div>
                        </div>
                    )}
                </div>
