...
export const getAllLabs = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (userRole === 'facilitator') {
            const user = await User.findById(userId).select('enrolledLabs');
            if (!user) {
                return res.status(404).json({ message: 'User not found' });
            }

            // Show only enrolled labs
            query = {
                ...query,
                _id: { $in: user.enrolledLabs }
            };
        }
...
export const startLab = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (req.user?.role === 'facilitator') {
            const user = await User.findById(userId).select('enrolledLabs');
            const isEnrolled = user?.enrolledLabs.some(id => id.toString() === labId);
            if (!isEnrolled) {
        ...
        // Trigger Automation Hooks (D10)
        await automationService.triggerHooks('lab_provision', {
            userId,
            labId,
...
export const stopLab = async (req: AuthRequest, res: Response) => {
    try {
        ...
        // Trigger Automation Hooks (D10)
        await automationService.triggerHooks('lab_deprovision', {
            userId,
            sessionId: session._id,
        });
